...
- [ ] T064 Run `npm run test:e2e` against an Expo web build on port 8082, per CLAUDE.md §12. Fix flakes deterministically — no `waitForTimeout` (CLAUDE.md §12 prohibition).
...
- [ ] T068 Validate SC-001 (zero data loss under 30 % packet loss for 4 h). Automated validation is limited to the unit-level property tests (T028, T030, T036–T042) which prove the zero-loss **logic** under mocked failures. A full physical soak against a real backend + toxiproxy is documented in `quickstart.md §10` as a manual procedure — run it once before the first production release and record the outcome in a short note at the bottom of `quickstart.md`.
